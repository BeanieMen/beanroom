...
It is structured in channels with multiple people in channels.
...
Please dont make random folders please
...
It:
- reads host SSH keys (`ssh_host_ed25519_key`) or wherever you where you wanna init the ssh key
...
```
SSH input → text → Broadcast → Terminal Renderer → SSH output
```
